...
class ReleaseController extends Controller
{
    ...
    public function add(){
        ...
        return view('contents.release.add', compact('titles', 'publishers'));
    }
    ...
    public function store(ReleaseRequest $req){
        ...
        if(!$this->release_interface->checkIsRecordExists($data)){
            ...
            return redirect()->route('release.index')->withSuccess(['Release created successfully']);
        }
        else{
            return redirect()->route('release.index')->withErrors(['Release already exists']);
        }
    }
    ...
    public function edit($id){
        ...
        if($release)
            return view('contents.release.edit', compact('release', 'id', 'titles', 'publishers'));
        else
            return redirect()->route('release.index')->withErrors(['Unknown release!']);
    }
    ...
    public function save(ReleaseRequest $req, $id){
        ...
        if(!$this->release_interface->checkIsRecordExists($data)){
            ...
            return redirect()->route('release.index')->withSuccess(['Release edited successfully']);
        }
        else{
            return redirect()->route('release.index')->withErrors(['Release already exists']);
        }
